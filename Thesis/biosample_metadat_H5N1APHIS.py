...
#Update to cooperate with the APHIS Specific biosample metadata, because it is linked weird
#Christie Woodside

'''This code is for the APHIS H5N1 computations specifically. some of the biosample doesn't really 
...
def bsDataGet(bs_term, sleeptime, bco_id):
    ...
    # i = Entrez.efetch(db='', id=SRA_id, rettype="gb", retmode="xml")
    # time.sleep(sleeptime)
    # record = Entrez.read(i)
    # time.sleep(sleeptime)
    # print("----\n\n", record)
...
def getLin(l_term, sleeptime):
    search = Entrez.esearch(db='biosample', term=l_term, retmode='xml', idtype="acc")
    time.sleep(sleeptime)
    record = Entrez.read(search)
    time.sleep(sleeptime)
    ...
    for filename in os.listdir(args.folder):
        if filename.endswith('.json'):
            ...
            with open(file_path, 'r') as json_file:
                ...
                # print("   ", assembly)
                # print(biosample_id)
                print(f"Processing biosample: {biosample_id}, assembly: {assembly}")
                ...
                for bs_data in bs_data_list:
                    bs_data["genome_assembly_id"] = assembly
