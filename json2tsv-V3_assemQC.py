#Newly Updated json2tsv-assemQC.py file as of October 28, 2024
...
This code will take the JSON file QC output for assemblyQC from schema v1.6 stored in a local folder and reformat it into a combined tsv. 
The output tsv will be pasted and aligned with the columns/headers for assemblyQC_HIVE (BCO ID ARGOS_000087) 
found in the data.argosdb.org dataset. This code was used for the datapush to ARGOSdb"""
#Now takes in the genome assemblyID at the top of the JSON to be used, instead of searching in Entrez and taking extra time
...
'''Example entry: 
python3 json2tsv-V2assemQC.py -t test_assembly.tsv --schema /Users/christiewoodside/desktop/argos/data_push_june/all --email christie.woodside@email.gwu.edu'''

'''Christie Example:
python3 json2tsv-V3_assemQC.py -t /Users/christiewoodside/desktop/argos/oct_28/all/ClostridiumPerf_Oct28_assembly.tsv --schema /Users/christiewoodside/desktop/argos/oct_28/all --email christie.woodside@email.gwu.edu --api bfbde99c962d228023e8d62a078bdb12d108'''
...
def getGenomicSection(l_term, sleeptime):
    ...
    if record["IdList"]:
        ...
        if ',' in definition:
            ...
            if 'plasmid unnamed' in definition:
                for i in range(1, 14):
                    if f'plasmid unnamed{i}' in definition:
                        return f'plasmid unnamed{i}'
                return 'plasmid unnamed'
            
            if 'chromosome' in definition:
                return 'chromosome'
            
            if 'unnamed' in definition:
                for i in range(1, 14):
                    if f'unnamed{i}' in definition:
                        return f'unnamed{i}'
                return 'unnamed'
            return result
    return "No ID found"
...
def parse_json(data):
    # Extract biosample and assembly IDs at the top level
    biosample = data.get("biosample", "")
    assembly = data.get("assembly", "")
    # Initialize an empty list to store each processed record
    columns_assembly = []
    # Append this record to columns_assembly
    columns_assembly.append()
    
    # Optional print for debugging
    print(record)
    return columns_assembly
...
def make_tsv(options):
    ...
    def get_data_from_flat_item(flat_item, key):
        ...
        #     print(flat_item.get("assembly",''))
        #     return flat_item.get("assembly")
    ...
    for schema in glob.glob(os.path.join(options.schema, '*.json')):
        with open(schema, "r") as jsonfile:
            ...
            data['biosample'] = 
            print(data)
            #print("Data structure:", data[columns_data["top_level"]])
    ...
    with open(options.tsv, "w", newline="") as tsvfile:
        ...
        writer.writerow(columns_data["top_level"])
        ...
        for schema in glob.glob(os.path.join(options.schema, '*.json')):
            with open(schema, "r") as jsonfile:
                ...
                for item in data[columns_data["top_level"]]:
                    flat_item = flatten_json(item)
                    print(flat_item)
                    assembly_id = flat_item.get("assembly", "")
                    print(assembly_id)
                    row = [get_data_from_flat_item(flat_item, key) for key in columns_data["columns"]]
                    row[columns_data["columns"].index("genome_assembly_id")] = assembly_id
                    writer.writerow(row)
