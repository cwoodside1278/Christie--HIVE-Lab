...
'''python3 json2tsv-V2_ngsQC.py -t /Users/christiewoodside/Desktop/ARGOS/oct_H5N1/ngs/H5N1_Texas_ngs_V1.tsv --schema /Users/christiewoodside/Desktop/ARGOS/oct_H5N1/ngs --email christie.woodside@email.gwu.edu --api bfbde99c962d228023e8d62a078bdb12d108'''
...
#     # Open the TSV file to write the output
#     with open(options.tsv, "w", newline="") as tsvfile:
#         writer = csv.writer(tsvfile, delimiter="\t")
#         writer.writerow(columns_data["columns"])
...
#         for schema in json_files:
#             with open(schema, "r") as jsonfile:
#                 # Use ijson to process the JSON in chunks
#                 parser = ijson.items(jsonfile, 'item')
...
#                     # Process each row based on columns_data
#                     row = process_row(flat_item, columns_data, srr_id, bs_id, bs_data_list)
#                     writer.writerow(row)

# def process_row(flat_item, columns_data, srr_id, bs_id, bs_data_list):
#     """
#     Helper function to process the row logic for make_tsv
#     """
#     row = []

#     # Iterate through each column defined in columns_data
#     for key in columns_data["columns"]:
#         if key == "biosample":
#             row.append(bs_id if bs_id else "-")
#         elif key == "genome_assembly_id":
...
#             row.append("SRA")
#         else:
#             # If the key is found in the biosample data, append it
#             if bs_data_list and key in bs_data_list[0]:
#                 row.append(bs_data_list[0][key])
#             else:
#                 # If the key is in the header_map, replace it with the mapped key
#                 if key in columns_data["header_map"]:
#                     key = columns_data["header_map"][key]
#                 # Add the data from flat_item (JSON data)
#                 row.append(flat_item.get(key, ""))
