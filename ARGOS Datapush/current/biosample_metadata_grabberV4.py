...
def bsDataGet(bs_term, genome_assembly_id, srr_id, sleeptime, bco_id):
    ...
    #print('List of UIDS: ', record['IdList'])
    time.sleep(sleeptime)
    bs_id = record['IdList'][0]                           #may need to change every now and then if it grabs the wrong uid. Change referring to the index.
    #print(bs_id)
    ...
    #This is the section of new code that distinguishes between v3 and v4
    ...
    #sra_id_list = [s.strip() for s in srr_id.split(',') if s.strip()]   #BIG NOTE; sometimes this may need to be switched between , and ;
    ...
    for sra_id in sra_id_list:
        ...
        if not search_record['IdList']:
            #print(search_record)
            print(f"DEBUG: Could not find SRR ID: {sra_id}. May need to switch line 192 to ; or ,")
